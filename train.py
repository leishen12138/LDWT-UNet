...
from LDWTUNet import LDWTUNet, LearnableDWT
...
parser.add_argument("--txt_root", type=str, required=True,
                    help="path to the text tag directory")  # 新增
...
def main(args):    
    dataset = FullDataset(args.train_image_path, args.train_mask_path, 352, mode='train', txt_root=args.txt_root)
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=18)
    device = torch.device("cuda")
    model = LDWTUNet(args.hiera_path)
    model.to(device)
    ...
    for epoch in range(args.epoch):
        for i, batch in enumerate(dataloader):
            ...
            optim.zero_grad()
            pred0, pred1, pred2 = model(x, labels)
